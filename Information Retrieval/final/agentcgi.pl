...
#If we're running in text mode, load up the interface
...
<strong><em><span style="font-size:20px">TravelBot</span></em></strong>
<BR><BR>
	
<strong><em><span style="font-size:14px">Your Results</span></em></strong>
<BR>
EndOfHeader
;
	...
	if (defined $query->param("flights")) {
	print "<strong>Flight Info</strong><BR>";
	&itn;
	print "<BR><BR>";}
	
	if (defined $query->param("drive")) {
	print "<strong>Driving Info</strong><BR>";
	&mapquest(0);
	print "<BR><BR>";}
	
	if (defined $query->param("train")) {
	print "<strong>Train Info</strong><BR>";
	&amtrak;
	print "<BR><BR>";
	}
	
	if (defined $query->param("destinfo")) {
	print "<strong>Destination Info</strong><BR>";
	&mapquest(1);
	}
	
	print << "EndOfFooter";
<BR>
<a href="http://hops.cs.jhu.edu/~sanford/cgi-bin/log.txt">log_file</a>
<BR>
<a href="http://hops.cs.jhu.edu/~sanford/index.html">Back to Main</a>
</BODY>
</HTML>
EndOfFooter
;
...
sub get_settings_html
{
#first get the round trip boolean
	if (defined $query->param("round")) {$round_trip = 1;}
	else {$round_trip = 0;}
		

# Get departure and return dates
	$depart_month = &get_month($query->param("leaveMonth"));
	$depart_day = $query->param("leaveDay");

	$return_month = &get_month($query->param("returnMonth"));
	$return_day = $query->param("returnDay");

# Origin data

	$origin = $query->param("from");
...
# Destination data
	...
	$destination = $query->param("to");
	my @destination = split(/,\s+/, $destination);
	$dest_city = $destination[0];
	$dest_city =~ s/\s/\+/g;
	$dest_state = $destination[1];

#Use non-default costs
	if (!defined $query->param("useDefaults")) {
	
	$SAC_van_cost = $query->param("sacCost");
	$SAC_van_mileage = $query->param("sacMPG");
	$vehicle_mileage = $query->param("otherMPG");
	$gas_cost = $query->param("gasPrice");
...
# Get Continental lowest price
	...
	$input = $URL."?stamp=NEWCOOKY*itn%2Ford%3DNEWREC,itn/air/united&airline=Continental&persons=1&air_avail=10&depart=$origin&dest.0=$destination&mon_abbr.0=$depart_month&date.0=$depart_day&hour_ampm.0=12 
pm&mon_abbr.1=$return_month&date.1=$return_day&hour_ampm.1=12 
pm&rt_ow=$rt_ow&best_itins=2&return_to=best_itins";
	&get_fare("Continental", $input);

# Get Delta lowest price

	$input = $URL."?stamp=NEWCOOKY*itn%2Ford%3DNEWREC,itn/air/united&airline=Delta&persons=1&air_avail=10&depart=$origin&dest.0=$destination&mon_abbr.0=$depart_month&date.0=$depart_day&hour_ampm.0=12 
pm&mon_abbr.1=$return_month&date.1=$return_day&hour_ampm.1=12 
pm&rt_ow=$rt_ow&best_itins=2&return_to=best_itins";
...
# Get United lowest price
	...
	$input = $URL."?stamp=NEWCOOKY*itn%2Ford%3DNEWREC,itn/air/united&airline=United&persons=1&air_avail=10&depart=$origin&dest.0=$destination&mon_abbr.0=$depart_month&date.0=$depart_day&hour_ampm.0=12 
pm&mon_abbr.1=$return_month&date.1=$return_day&hour_ampm.1=12 
pm&rt_ow=$rt_ow&best_itins=2&return_to=best_itins";
...
# Get station abbreviations
	...
	$input = "inputDepartureDate=$depart_month $depart_day, 2003&inputDepartureStation=$origin&inputDestinationStation=$destination&inputDepartureTime=1200&inputNumberOfTravelers=1";
...
# Input data
	...
	if ($round_trip == 0)
	{
		$input = "inputDepartureDate=$depart_month $depart_day, 2003&inputDepartureStation=$dep_station&inputDestinationStation=$dest_station&inputDepartureTime=1200&inputNumberOfTravelers=1";
	}
	else
	{
		$input = "inputDepartureDate=$depart_month $depart_day, 2003&inputReturnDate=$return_month $return_day, 2003&inputDepartureStation=$dep_station&inputDestinationStation=$dest_station&inputDepartureTime=1200&inputReturnTime=1200&inputNumberOfTravelers=1";
	}
...
# Output
	...
	&output("\nTotal Driving Distance: ".$distance.$eol);
	&output("Estimated Driving Time:".$time.$eol);
	
	if (!defined $query->param("to")) 
	{
		&output("Driving Directions Saved To Directions.txt".$eol);
	}
	else 
	{
		&output("Driving Directions Saved To <a href=\"http://hops.cs.jhu.edu/~sanford/cgi-bin/directions.txt\">Directions.txt</a><BR>");
	}
	

	my $rentalCost = sprintf("%.2f",$distance * $SAC_van_cost);
	my $sacvanCost = sprintf("%.2f",$distance / $SAC_van_mileage);
	my $gasCost = sprintf("%.2f",$distance / $vehicle_mileage);
	
	&output("\nSAC Van Rental Cost: \$".($rentalCost).$eol);
	&output("SAC Van Gas Cost: \$".($sacvanCost).$eol);
	&output("Vehicle Gas Cost: \$".($gasCost).$eol);
}
...
# Get Lodging sites
	...
	{
		&output("Addresses + Phone #'s Saved To <a href=\"http://hops.cs.jhu.edu/~sanford/cgi-bin/info.txt\">Info.txt</a><BR>");
	}
}
...
# POST (via GET) data and parse returned price
	...
	if (scalar @prices == 0)
	{
		&output("No match found for $airline$eol");
		return;
	}
...
# Output lowest price
	...
	&output("$airline Lowest Cost: ".$prices[0]."$eol");
}
...
# Stupid Amtrak lists BWI as the default Baltimore station; this is a hack fix

	if ($city eq "Baltimore" and $state eq "MD")
	{ return "BAL"; }

# Get the first letter of the city

	$city =~ s/\+/ /g;
	my $first_letter = substr($city, 0, 1);

# Get Amtrak's station listing (alphabetical by city)

	my $list = &GET("http://tickets.amtrak.com/slc$first_letter.htm");
	$list =~ /$city.+$state<.+>\n<.+>([A-Z]{3})/;
