...
public class SimnetCryptoEngine {
  ...
  public byte [] decryptAESCBC(byte [] in, byte [] key)
	...
	/** Make a random array of bytes of size MD5_DIGEST_SIZE
		*
		* @return byte[] - a byte array containing the mac key
		*/
	public byte [] makeMacKey(){
		byte [] out = new byte[MD5_DIGEST_SIZE];
		rand.nextBytes(out);
		return out;
	}

	public AsymmetricCipherKeyPair generateDSAKeys() {
		System.out.println("DSA Key Generation");
		DSAParametersGenerator paramsGen = new DSAParametersGenerator();
		paramsGen.init(768, 3, rand);
		DSAParameters params = paramsGen.generateParameters();
		DSAKeyGenerationParameters keyGenParams = new DSAKeyGenerationParameters(rand, params);
		DSAKeyPairGenerator keyGen = new DSAKeyPairGenerator();
		keyGen.init(keyGenParams);
		return keyGen.generateKeyPair();
	}

	public DSASignature DSASignObject(Object o, DSAPrivateKeyParameters privateKey) {
		System.out.println("DSA Signing");
		DSASigner signer = new DSASigner();
		signer.init(true, privateKey);
		return new DSASignature(signer.generateSignature(getSHA1Digest(serialize(o))));
	}

	public boolean DSAVerifyObjectSignature(Object o, DSASignature sig, DSAPublicKeyParameters publicKey) {
		System.out.println("DSA Signature Verification");
		DSASigner signer = new DSASigner();
		signer.init(false, publicKey);
		return signer.verifySignature(getSHA1Digest(serialize(o)), sig.r, sig.s);
	}

	public AsymmetricCipherKeyPair generateRSAKeys() {
		System.out.println("RSA Key Generation");
		BigInteger two = BigInteger.ONE.add(BigInteger.ONE);
		BigInteger exp = (two.pow(16)).add(BigInteger.ONE);
		RSAKeyGenerationParameters keyGenParams = new RSAKeyGenerationParameters(exp, rand, 768, 3);
		RSAKeyPairGenerator keyGen = new RSAKeyPairGenerator();
		keyGen.init(keyGenParams);
		return keyGen.generateKeyPair();
	}

	public byte[] HMacMD5Object(Object o, byte[] key) {
		return HMacMD5(serialize(o), key);
	}
}
