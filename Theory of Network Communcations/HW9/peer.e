...
def makePeer(myName) :any {
	...
	def peer {
		...
		to EQmoveDown(x) :any {
			if (data == null) {
				super <- print("Enqueued " + x + " at peer " + myName)
				data := x
				super <- addFullPeer(peer, x)
			} else {
		...
		to DQmoveDown() :any {
			...
			when (lchildFullVow, rchildFullVow) -> doneDQ(lchildFull, rchildFull) :any {
				if (lchildFull == true && rchildFull == true) {
					if (entropy.nextInt(2) == 0) {
						data := lchild <- DQmoveDown()
						super <- addFullPeer(peer, data)
					} else {
						data := rchild <- DQmoveDown()
						super <- addFullPeer(peer, data)
					}
