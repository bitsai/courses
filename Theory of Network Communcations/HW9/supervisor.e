...
def makeSupervisor(myName) :any {
	...
	def supervisor {
		...
		to dequeue() :any {
			if (fullPeers.size() == 0) {
				supervisor <- print("Nothing to dequeue!")
			} else {
				var index := 0
				def targetIndex := entropy.nextInt(fullPeers.size())
				var xVow := null
				var peerNameVow := null

				for peer => data in fullPeers {
					if (index == targetIndex) {
						xVow := peer <- DQmoveDown()
						peerNameVow := peer <- getName()
					}

					index += 1
				}

				when (xVow, peerNameVow) -> doneDQ(x, peerName) :any {
					supervisor <- print("Dequeued " + x + " from peer " + peerName)
				} catch e {}
			}
		}
		
		to addFullPeer(peer, x) :any {
			fullPeers.put(peer, x)
		}
		
		to removeFullPeer(peer) :any {
			fullPeers.removeKey(peer)
		}
